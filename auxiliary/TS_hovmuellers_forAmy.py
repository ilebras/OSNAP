...
dat=pickle.load(open(datadir+'OSNAP2016recovery/pickles/xarray/CF_M_2014-2016_hourlyTSD_1903.pickle','rb'))

eddy_stats=io.loadmat(datadir+'OSNAP2016recovery/Eddies/CF_eddy_stats.mat')
...
for ii in [7]:
    nnind=~isnan(dat[ii].depth[0,:])
    datemat=tile(dcheck,[len(dat[ii].dpvec),1])
    if ii==7:
        dpind=(dat[7].dpvec.values!=500)
    else:
        dpind=(dat[ii].dpvec.values!=1e6)
    figure(figsize=(16,4))
    contourf(datemat[:,nnind][dpind,:],dat[ii].depth.values[:,nnind][dpind,:],dat[ii].ptmp.values[:,nnind][dpind,:],cmap=cm.RdYlBu_r,levels=arange(1.5,6.1,0.1),extend='both')
    colorbar(ticks=range(0,7,1),label='Potential temperature [$^\circ$ C]')
    contour(datemat[:,nnind][dpind,:],dat[ii].depth.values[:,nnind][dpind,:],dat[ii].sigma0.values[:,nnind][dpind,:],colors='k',levels=arange(27.6,27.9,0.05))
    plot(eddy_date[(loc_cycl[ii-5,:]>0)==True],0*ones(sum(loc_cycl[ii-5,:]>0)),'ko')
    ylabel('depth [m]')
    title('CF'+str(ii))
    for dd in range(len(dlist[:-1])):
        xlim(dlist[dd],dlist[dd+1])
        savefig('../figures/Eddies/hov/Tdenhov_abs_CF'+str(ii)+'_d'+str(dd)+'.png',bbox_inches='tight',dpi=300)
        savefig('../figures/Eddies/hov/Tdenhov_abs_CF'+str(ii)+'_d'+str(dd)+'.pdf',bbox_inches='tight')
        # savefig('../figures/Eddies/hov/Tdenhov_abs_CF'+str(ii)+'_d'+str(dd)+'.ps',bbox_inches='tight')
