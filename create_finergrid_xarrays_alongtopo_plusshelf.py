...
# Note: geostrophic velocity found above can be placed at -10km instead of -5km because it would be using the same horizontal gradient
shdat
...
# 4. now inerpolate vertically once more to fill in bottom triangles...
for vv in dat:
    if vv[0]!='d':
        for tt,adate in enumerate(dat.date):
...
pickle.dump(newgrid,open('../pickles/xarray/CF_xarray_gridplot_notid_1804shelf.pickle','wb'))
