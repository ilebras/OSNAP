...
trans14_moorloc=nansum(adcp_14['across track velocity'][:,moorind[-2]:moorind[-1]]*depthdiffmat_a14[:,moorind[-2]:moorind[-1]]*middistmat_a14[:,moorind[-2]:moorind[-1]])/1e3
trans16_moorloc=nansum(adcp_16['across track velocity'][:,moorind[-2]:moorind[-1]]*depthdiffmat_a16[:,moorind[-2]:moorind[-1]]*middistmat_a16[:,moorind[-2]:moorind[-1]])/1e3
...
plt4pan()
#################################################################################
# Do the same for geostrophic velocity from CTD (just using plotting framework, really)
#################################################################################

[prs_ctd,dist,lon,lat,sal,tmp,pden,geoshear,geovel]=pickle.load(open('../pickles/Shipboard/CTD_1416geo.pickle','rb'))

#mooring version for longer mean!

def contmoorave_min(axspec,date1,date2):
    im=axspec.contourf(dat.distance,dat.depth,dat['across track velocity'].sel(date=slice(date1,date2)).mean(dim='date').T,arange(-0.8,0.805,0.05),cmap=cm.RdBu_r);
    axspec.contour(dat.distance,dat.depth,dat['across track velocity'].sel(date=slice(date1,date2)).mean(dim='date').T,[-0.4,-0.2,0],colors='k');
    axspec.fill_between(bathdist,bathbath,1800*ones(len(bathbath)),color='k',zorder=22)
    [axspec.axvline(mm,color='w',linewidth=2) for mm in distvec]
    [axspec.axvline(mm,color='k',linewidth=0.8) for mm in distvec]
    return im
...
def contgeo_min(axspec,key):
    axspec.contourf(dist[key][:-1],prs_ctd,geovel[key],arange(-0.8,0.805,0.05),cmap=cm.RdBu_r);
...
def plt4pan_geo():
    fs=11
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, sharex=True, sharey=True,figsize=(8,6))
    contgeo_min(ax1,'2014_3')
    ax1.set_xlim([-15,110])
    ax1.set_ylim([800,0])
    im=contmoorave_min(ax2,'2014-08-07','2014-09-07')
    contgeo_min(ax3,'2016_6')
    im=contmoorave_min(ax4,'2016-06-28','2016-07-28')
    ...
    savefig('../figures/shipboard/GEOmoorcomp_4panel_1803bathy.pdf')
    savefig('../figures/shipboard/GEOmoorcomp_4panel_1803bathy.png')

plt4pan_geo()

for key in lat:
    # figure()
    # plot(lon[key],lat[key],'o')
    # title(key)
    figure()
    contourf(geovel[key],prs_ctd)
    gca().invert_yaxis()
    colorbar()
xxxxxxx
