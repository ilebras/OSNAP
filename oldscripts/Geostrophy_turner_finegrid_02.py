...
for vv in newgrid:
    if vv[0]!='d':
        print(vv)
        for dd,adist in enumerate(datin.distance):
            datin[vv][dd,:,:]=datin[vv][dd,:,:].where(datin[vv][dd,:,:].depth<=bathonmygrid[dd])
...
for tt,na in enumerate(datin.date):
    minind[tt]=where(max(avelint[2:15,0,tt])==avelint[2:15,0,tt])[0][0]+2
...
dat=xr.Dataset({'geostrophic velocity': (['distance', 'depth', 'date'],  geovel-0.05),
...
# cc_turner=dat['turner angle'].copy()
# for tt,mm in enumerate(minind):
#         cc_turner[mm:,:,tt]=NaN
...
# eg_turner=dat['turner angle'].copy()
# for tt,mm in enumerate(minind):
#         eg_turner[:mm,:,tt]=NaN
...
cc['baro trans']=cc['abs trans']-cc['geo trans']
# cc['turner']=cc_turner.mean('depth').mean('distance')
...
eg['baro trans']=eg['abs trans']-eg['geo trans']
# eg['turner']=eg_turner.mean('depth').mean('distance')

# figure(figsize=(12,3))
# cc['turner'].plot(color=ccol,label='Coastal Current')
# eg['turner'].plot(color=egicol,label='Slope Current')
# legend()
# [axhline(ii,color='k',alpha=0.5) for ii in range(-45,40,45)]
# ylabel('Turner Angle [$^\circ$]',)
# yticks(range(-90,50,45))
# ylim([-90,45])
# savefig('../figures/geocomp/turner_tseries.png',bbox_inches='tight')
#
# def velcomplot(dic,colc,tit,ylimi):
#     figure(figsize=(12,3))
#     dic['abs trans'].plot(color=colc,linestyle='-',label='',alpha=0.8)
#     dic['geo trans'].plot(color=colc,linestyle='--',label='',alpha=0.8)
#     dic['abs trans'].resample('M',dim='date').plot(color=colc,linestyle='-',label='absolute transport')
#     dic['geo trans'].resample('M',dim='date').plot(color=colc,linestyle='--',label='geostrophic transport')
#     legend()
#     title(tit)
#     ylim([ylimi,0])
#     ylabel('Transport [Sv]')
#     savefig('../figures/geocomp/geotranscomp_'+tit+'.png',bbox_inches='tight')
#
#
# def velcompall(dic,colc,tit,ylimi):
#     figure(figsize=(12,3))
#     dic['abs trans'].plot(color=colc,linestyle='-',label='',alpha=0.8)
#     dic['geo trans'].plot(color=colc,linestyle='--',label='',alpha=0.8)
#     dic['abs trans'].resample('M',dim='date').plot(color=colc,linestyle='-',label='absolute transport')
...
#     dic['baro trans'].resample('M',dim='date').plot(color=colc,linestyle='-.',label='"barotropic" transport')
#     legend()
#     title(tit)
#     ylim([ylimi,0])
#     ylabel('Transport [Sv]')
#     savefig('../figures/geocomp/transcompall_'+tit+'.png',bbox_inches='tight')
#
# def justgeo(dic,colc,tit,ylimi):
#     figure(figsize=(12,3))
#     dic['geo trans'].plot(color=colc,linestyle='-',label='',alpha=0.8)
#     dic['geo trans'].resample('M',dim='date').plot(color=colc,linestyle='-',label='geostrophic transport')
#     title(tit+' Geostrophic Transport')
#     ylim([ylimi,0])
#     ylabel('Transport [Sv]')
#     savefig('../figures/geocomp/justgeo_'+tit+'.png',bbox_inches='tight')
#
# def justbaro(dic,colc,tit,ylimi):
#     figure(figsize=(12,3))
#     dic['baro trans'].plot(color=colc,linestyle='-',label='',alpha=0.8)
#     dic['baro trans'].resample('M',dim='date').plot(color=colc,linestyle='-',label='"barotropic" transport')
#     title(tit+' Barotropic Transport')
#     ylim([ylimi,0])
#     ylabel('Transport [Sv]')
#     savefig('../figures/geocomp/justbaro_'+tit+'.png',bbox_inches='tight')
#


# justbaro(cc,ccol,'Coastal Current',-1.5)
#
# justgeo(cc,ccol,'Coastal Current',-1.5)
#
# velcompall(cc,ccol,'Coastal Current',-2.5)
#
# velcomplot(cc,ccol,'Coastal Current',-2.5)
#
# justbaro(eg,egicol,'Slope Current',-30)
#
# justgeo(eg,egicol,'Slope Current',-15)
#
# velcomplot(eg,egicol,'Slope Current',-40)
#
# velcompall(eg,egicol,'Slope Current',-40)

pickle.dump([cc,eg],open('../pickles/geoturndic_005.pickle','wb'))
...
def meanplot(field):
    ...
    savefig('../figures/sections/fullmean_fine_'+field+'_fulldpth.png',bbox_inches='tight')
    savefig('../figures/sections/fullmean_fine_'+field+'_fulldpth.pdf',bbox_inches='tight')
...
def stdplot(field,limoverride):
    ...
    colorbar(ax1,label=univec[field][4])
    savefig('../figures/sections/fullstd_fine_'+field+'_fulldpth.png',bbox_inches='tight')
    savefig('../figures/sections/fullstd_fine_'+field+'_fulldpth.pdf',bbox_inches='tight')
...
def fourplot(field,secondyear=0):
    ...
    savefig('../figures/sections/seasonal_fine_'+field+'_'+datename+'year.png',bbox_inches='tight')
    # savefig('../figures/sections/seasonal_fine_'+field+'_'+datename+'year.pdf')
    # figure(figsize=(12,6))
    for nn in range(1,5):
        subplot(2,2,nn)
        ylim([400,0])
    ...
    savefig('../figures/sections/seasonal_fine_'+field+'_'+datename+'year_zoom.png',bbox_inches='tight')
    # savefig('../figures/sections/seasonal_fine_'+field+'_'+datename+'year_zoom.pdf')
...
def seasonline(field,dpchoice,ylab,tit,atype,savename,ychoice='both',tu=0):
    ...
    savefig('../figures/seasonal/seasonline_fine_'+savename+'_'+ychoice+'.png',bbox_inches='tight')
