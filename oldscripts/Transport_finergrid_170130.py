...
salims=[34,34.8,34.85,34.875,34.9,34.925,34.95]

#first, just velocities in salinity space:

saltrans=zeros((len(salims)+2,len(daily.date)))

Made this +2 again --> need to just label inbetween (i.e. put labels on 0.5++1) and label those
Go through and fix.
...
figure(figsize=(14,4))
contourf(daily.date,range(len(salims)+1),salvel,21,cmap=cm.hot,vmin=-0.8,vmax=0);
yticks(range(len(salims)+1),hstack(('<',[str(ss) for ss in salims],'>')))
colorbar()

figure(figsize=(14,4))
contourf(daily.date,range(len(salims)+1),saltrans,21,cmap=cm.hot);
yticks(range(len(salims)+1),hstack(('<',[str(ss) for ss in salims],'>')))
colorbar()
