...
#Get into an xarray
grdat=xr.Dataset({'salinity': (['pressure','distance', 'occupation'],  saltot),
                    'temperature': (['pressure','distance', 'occupation'],  tmptot),
                    'density': (['pressure','distance', 'occupation'],  pdentot),},
                coords={'distance': distgrid,
                        'pressure': smprs,
                        'occupation': occvec})

grdat['density'].sel(occupation='2014 (KN221)').plot()


pickle.dump(grdat,open('../pickles/Shipboard/CTD_xarray_1806_relmoor.pickle','wb'))
