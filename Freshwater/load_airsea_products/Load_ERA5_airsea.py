...
dat.tp.mean(dim='time').where(mymask==0).plot()
cut_dat=dat.where(mymask==0)

cut_dat.tp.mean(dim='time').plot()
...
(cut_dat.slhf_int+cut_dat.sshf_int).plot()
...
cut_dat.to_netcdf(datadir+'aux_data/Air-Sea_FW/ERA5_ilebras/ERA5_from2000_ephf_cutint_ilebras.nc','w')
