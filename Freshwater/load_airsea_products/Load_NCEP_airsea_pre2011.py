...
## CFS (2000-2011)
#eminusp
eplist=sort(glob.glob(datadir+'aux_data/Air-Sea_FW/NCEP_CFS/Average/EminusP/*.nc'))

ep=xr.open_dataset(eplist[0])
for ii,xx in enumerate(eplist[1:]):
    eptmp=xr.open_dataset(xx)
    ep=xr.concat([ep,eptmp],dim='time')
ep=ep.rename({'EMNP_L1_FcstAvg1hr':'ep'})

ep=ep.sortby('time').resample(time='1M').mean(dim='time')
ep
#surfacefluxes
hflist=sort(glob.glob(datadir+'aux_data/Air-Sea_FW/NCEP_CFS/Average/SurfaceFluxes/*.nc'))
...
int_all.to_netcdf(datadir+'aux_data/Air-Sea_FW/NCEP_CFS/NCEP_pre2011_ephf_cutint_ilebras.nc','w')
