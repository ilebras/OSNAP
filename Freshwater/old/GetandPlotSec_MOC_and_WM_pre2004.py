...
def plot_TS_obs_splitboth():
    f,[ax1,ax2]=subplots(1,2,figsize=(10,3.75),sharex=True,sharey=True)
    for wm in ['AWS','PWS','DWS']:
        ax1.scatter(WM_obs['PSAL'].sel(WM=wm).mean(dim='TIME').values,WM_obs['PTMP'].sel(WM=wm).mean(dim='TIME').values,
                s=WM_obs['TRANS'].sel(WM=wm).mean(dim='TIME').values**2*6,linewidth=3,label=wm,color=coldic[wm],zorder=100,alpha=0.8)
    for wm in ['AWN','PWN']:
        ax2.scatter(WM_obs['PSAL'].sel(WM=wm).mean(dim='TIME').values,WM_obs['PTMP'].sel(WM=wm).mean(dim='TIME').values,
                s=WM_obs['TRANS'].sel(WM=wm).mean(dim='TIME').values**2*6,linewidth=3,label=wm,color=coldic[wm],zorder=100,alpha=0.8)
    ...
    # f.suptitle('Transport-weighted water mass properties\n\n',fontsize=16)
    ax1.set_title('Southern boundary: OSNAP',fontsize=16)
    ax2.set_title('Northern boundary',fontsize=16)
    savefig(figdir_paper+'TS_obs_only_split.png',bbox_inches='tight')
    savefig(figdir_paper+'TS_obs_only_split.pdf',bbox_inches='tight')
plot_TS_obs_splitboth()
...
############################################################################################
################  MAURITZEN TS PLOT PLUS GRAVEYARD OF OTHER PLOTS  #############################################
############################################################################################
#
# M96={}
