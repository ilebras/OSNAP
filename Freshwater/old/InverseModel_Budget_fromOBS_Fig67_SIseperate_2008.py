...
def get_U_S_T_from_WM(WM):
    ...
    U['SI']=-noresm['mltfrz'].mean(dim='time').values
    U['FW']=(-noresm['runoff']-noresm['ep_int']).mean(dim='time').values
    U['Q']=-noresm['hf_int'].mean(dim='time').values*tera/rhow/cp/1e6 #1e6 for the Sverdrups
    U_std['SI']=noresm['mltfrz'].std(dim='time').values
    U_std['FW']=(-noresm['runoff']-noresm['ep_int']).std(dim='time').values
...
wmvec=['PWS','AWS','DWS','PWN','AWN','FW','SI','Q']
...
def plot_base_case_simple(Ubase,Ue,U,plt):
    ...
    axx[2].bar(range(1),U['SI']+U['FW'],color=coldic['FW'],yerr=Ue['SI']+Ue['FW'],capsize=capi,alpha=alf)
    axx[2].plot(range(1),Ubase['SI']+Ubase['FW'],'o',color='k')
    fwlim=0.2
    axx[2].set_ylim(-fwlim,fwlim)
    for yrS in ['2015','2016','2017']:
        for yrN in ['2006','2007','2008','2009','2010']:
            axx[2].plot(0.2,Usol_yr[yrS+'/'+yrN]['FW']+Usol_yr[yrS+'/'+yrN]['SI'],'o',color='w',mec='k')
...
def get_split(U_choose,a_choose,b_choose):
    ...
    split['FW']['Est']=FWfrac_west*(U_choose['FW']+U_choose['SI'])*1e3
    split['FW']['AW']=(1-a_choose[0]-b_choose[0])*(U_choose['FW']+U_choose['SI'])*1e3 #doesn't matter which a and b you choose (as long as same index), will all have same sum.
    split['FW']['Ov']=FWfrac_east*(U_choose['FW']+U_choose['SI'])*1e3-split['FW']['AW']
    # given this distribution, how much PWN flows into PWS?
    split['PWN']['PWS']=U_choose['PWN']*(split['FW']['Est']-(U_choose['FW']+U_choose['SI'])*1e3*a_choose[-1])/(U_choose['FW']+U_choose['SI'])/1e3/(a_choose[0]-a_choose[-1])
...
def plot_adep_base():
    ...
    for ii,var in enumerate([a_pwmat,b_pwmat]):
        ...
        axx[ii].plot(xvar*Ubase['PWN'],var[:,10,10]*(Ubase['FW']+Ubase['SI'])*1e3,linewidth=4,color=basecol,label='Full mean solution',zorder=5)
        axx[ii].plot(xvar2*Ubase['PWN'],var[0,10,10]*(Ubase['FW']+Ubase['SI'])*1e3,'o',color=basecol,label='',zorder=5,mec='k',markersize=msize)
        axx[ii].plot(xvar3*Ubase['PWN'],var[-1,10,10]*(Ubase['FW']+Ubase['SI'])*1e3,ash,color=basecol,label='',zorder=5,mec='k',markersize=msize)
        axx[ii].set_ylim(0,120)
...
def plot_adep_pw():
    ...
    for ii,var in enumerate([a_pwmat,b_pwmat]):
        ...
        axx[ii].plot(xvar*Ubase['PWN'],var[:,10,10]*(Ubase['FW']+Ubase['SI'])*1e3,linewidth=4,color=basecol,label='Base case',zorder=5)
        axx[ii].plot(xvar*U_pw['PWN'],var[:,5,5]*(U_pw['FW']+U_pw['SI'])*1e3,color=pwcol,zorder=4,label='Polar Waters fresher by 0.5',linewidth=4)
        axx[ii].plot(xvar2*Ubase['PWN'],var[0,10,10]*(Ubase['FW']+Ubase['SI'])*1e3,'o',color=basecol,label='',zorder=5,markersize=msize,mec='k')
        axx[ii].plot(xvar2*U_pw['PWN'],var[0,5,5]*(U_pw['FW']+U_pw['SI'])*1e3,'o',color=pwcol,zorder=4,label='',mec='k',markersize=msize)
        axx[ii].plot(xvar3*Ubase['PWN'],var[-1,10,10]*(Ubase['FW']+Ubase['SI'])*1e3,ash,color=basecol,label='',zorder=5,markersize=msize,mec='k')
        axx[ii].plot(xvar3*U_pw['PWN'],var[-1,5,5]*(U_pw['FW']+U_pw['SI'])*1e3,ash,color=pwcol,zorder=4,label='',mec='k',markersize=msize)
        axx[ii].set_ylim(-10,140)
    axx[0].plot((1-epsilon)*U_fwvar['PWN'],a_fw*(U_fwvar['FW']+U_fwvar['SI'])*1e3,linewidth=4,color=fwcol,label='Add 20 mSv of Fresh Water')
    axx[1].plot(epsilon*U_fwvar['PWN'],b_fw*(U_fwvar['FW']+U_fwvar['SI'])*1e3,linewidth=4,color=fwcol)
    axx[0].plot(U_fwvar['PWN'],a_fw[0]*(U_fwvar['FW']+U_fwvar['SI'])*1e3,'o',color=fwcol,label='',mec='k',markersize=msize)
    axx[1].plot(0,b_fw[0]*(U_fwvar['FW']+U_fwvar['SI'])*1e3,'o',color=fwcol,label='',mec='k',markersize=msize)
    axx[0].plot(0,a_fw[-1]*(U_fwvar['FW']+U_fwvar['SI'])*1e3,ash,color=fwcol,label='',mec='k',markersize=msize)
    axx[1].plot(U_fwvar['PWN'],b_fw[-1]*(U_fwvar['FW']+U_fwvar['SI'])*1e3,ash,color=fwcol,label='',mec='k',markersize=msize)
    axx[0].plot(split_base['PWN']['PWS'],split_base['FW']['Est'],'*',color=basecol,label='',markersize=15,zorder=100,mec='k')
    axx[1].plot(split_base['PWN']['DWS'],split_base['FW']['Ov'],'*',color=basecol,label='',markersize=15,zorder=100,mec='k')
