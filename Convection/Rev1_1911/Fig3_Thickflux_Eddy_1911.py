...
OOI=xr.open_dataset(datadir+'OSNAP2016recovery/gridded_CF-OOI/ADCP_OOI_FLA_500m.nc')


for xxx in [CF5,CF6,M1,OOI]:
...
colvec=[cf5col,cf6col,m1col,ooicol][::-1]

moorvec=['CF5','CF6','M1','OOI'][::-1]
...
def cont_thick(field1,field2,axx,maxt,hlevs,labit):
    ...
    # axx.contourf(dendat.date.values,dendat.distance.values,field3.values, [0.75,1.25], colors='grey',alpha=0.5,hatches=['////'],)
    axx.clabel(dd,fmt='%d')
...
def plot_eddyvar(axx,tit):
    field='u'
    for ii,xx in enumerate([CF5,CF6,M1,OOI][::-1]):
        dstep=24
        if ii<2:
            elfieldo=xx[field]
        else:
        ...
        leg=axx.legend(loc=(1.05,0),fontsize=14)
...
def plot_thickhov_eddy():
    f,axi=subplots(3,1,figsize=(10,7),constrained_layout=True)
    ...
    for axx in axi:
        ...
        axx.xaxis.set_major_formatter(monthFMT)
    ...
    axi[-1].set_xticklabels(['October','January\n2015','April','July','October','January\n2016','April','July',])
    # axi[-1].xaxis.set_minor_formatter(monthFMT)
    # axi[-1].xaxis.set_major_formatter(monthFMT)
